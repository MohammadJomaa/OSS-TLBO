...
public class OSSTLBO {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < 100; i++) {
            OSSTLBO T = new OSSTLBO(10, 1000, 100, -100, 100, 0);
    ...
    public OSSTLBO(int PopulationCount, int MaxIteration, int NumberOf_Dimensions, double RangeMin, double RangeMax, double F_Min) {
    ...
    public double runAlgorythm() throws Exception {
        ...
        for (int i = 0; i < getPopulationCount(); i++) {
            students.set(i, OptimalStoppingRule(10000, 3333));
        }
        ...
        while (Iteration < MaxIteration) {
           ...
           /* System.out.println("------------------------");
            System.out.println(" cost of best solu is  " + Iteration + " (" + currentBestSolution.getCost() + ")");
            System.out.println("**************************** ");
*/
            teacherPhase();

            learnerPhase();
            Updateing_population();
            currentBestSolution = findBestStudent();
            currentBestSolutionFitValue = currentBestSolution.getCost();
            Iteration++;
    ...
    private void teacherPhase() {
        ...
        IntStream.range(0, getPopulationCount()).forEach(i -> {
            ...
            IntStream.range(0, getNumberOf_Dimensions()).forEach(j -> {
                ...
                double differenceTeacher_student = randomGenerator.nextDouble() * (currentBestSolution.getSolution()[j] - students.get(i).getSolution()[j]);

                newSolution.updateSolution(j, students.get(i).getSolution()[j] + differenceMean + differenceTeacher_student); // jomaa test
    ...
    private void Updateing_population() {
        SortPop();
        m = m + 1;

        if (m == 20) {
            m = 0;
            /*double Jr = randomDouble(0, 1);
            if (Jr < 0.5)*/
            DOL_Initialize_Population(false, 1, 0);
    ...
    private Student OptimalStoppingRule(int n, int r) {
        Student bestsolution = new Student(NumberOf_Dimensions, RangeMin, RangeMax, F_Min);
